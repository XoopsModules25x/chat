...
switch ( $act ) {
	...
	case "load" :
		...
		$start = 0;
		$limit = xoops_getModuleOption( 'startlimitmsg', $dirname );
		...
		$time_sleep = $time - xoops_getModuleOption( 'timesleep', $dirname );
		// Счётчик
		$i = 0;
		...
		$online_handler->write( $uid, $uname, $time, $xoopsModule->getVar('mid'), $ip );

		// Посылается JS, содержит ID последней записи, которая есть у пользователя
		$last_message_id = isset($_POST['last']) ? intval($_POST['last']) : 0;
		// Считываем из кеша ID последней месаги
		$messid = intval( XoopsCache::read( $cache_lm_key ) );
		...
		if( $last_message_id < $messid || $messid == 0 || $last_message_id == 0) {
			...
			if( $GLOBALS['xoopsDB']->getRowsNum($result) > 0 ) {
				...
				foreach ( $messages as $value ) {
					...
					// Подсветка мессаг
					if($value['uid'] == $uid) $bg_msg = 'chat-bg-mymsg';
					elseif(chat_memsg($value['message'], $uname)) $bg_msg = 'chat-bg-memsg';
					else $bg_msg = '';
